...
library BLS {
    ...
    function msm(G1Point[] memory points, uint256[] memory scalars) internal view returns (G1Point memory result) {
        /// @solidity memory-safe-assembly
        assembly {
            let k := mload(points)
    ...
    function msm(G2Point[] memory points, uint256[] memory scalars) internal view returns (G2Point memory result) {
        /// @solidity memory-safe-assembly
        assembly {
            let k := mload(points)
            if iszero(eq(k, mload(scalars))) {

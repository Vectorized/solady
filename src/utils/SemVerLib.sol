...
library SemVerLib {
    ...
    /// @dev The SemVer small string does not comply with the required format.
    error InvalidSemVer();
    ...
    function cmp(bytes32 a, bytes32 b) internal pure returns (uint256) {
        /// @solidity memory-safe-assembly
        assembly {
            function revertInvalidSemVer() {
                mstore(0x00, 0x26d865f4) // `InvalidSemVer()`.
                revert(0x1c, 0x04)
            }
            function parseReleaseComponent(i_, a_, v_) -> _r, _o {
                for { _o := i_ } 1 { _o := add(1, _o) } {
                    let digit_ := sub(byte(_o, a_), 48)
                    if iszero(lt(digit_, 10)) { break }
                    _r := add(mul(10, _r), digit_)
                }
                if and(lt(_o, 0x20), i_
            }
            let i := or(eq(byte(0, a), 0x76), eq(byte(0, a), 0x56))
            let j := or(eq(byte(0, b), 0x76), eq(byte(0, b), 0x56))
            let x := 0
            let y := 0
            x, i := parseReleaseComponent(i, a, ord('.'))

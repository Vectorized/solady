...
library EnumerableSetLib {
    ...
    function at(Bytes32Set storage set, uint256 i) internal view returns (bytes32 result) {
        ...
        assembly {
            ...
            mstore(0x04, _ENUMERABLE_WORD_SET_SLOT_SEED)
            mstore(0x00, set.slot)
            result := sload(add(keccak256(0x00, 0x24), i))
            result := mul(result, iszero(eq(result, _ZERO_SENTINEL)))
        }
    }
    ...
    function _toBytes32Set(Uint256Set storage set)
        private
        pure
        returns (Bytes32Set storage result)
    {
        /// @solidity memory-safe-assembly
        assembly {
            result.slot := set.slot
        }
    }

    /// @dev Casts to a Bytes32Set.
    function _toBytes32Set(Int256Set storage set)
        private
        pure
        returns (Bytes32Set storage result)
    {
    ...
    function _toInts(bytes32[] memory a) private pure returns (int256[] memory result) {
        /// @solidity memory-safe-assembly
        assembly {
            result := a
        }

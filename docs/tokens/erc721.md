...
Solady's ERC721 token implementation is an opinionated and highly optimized implementation of the [ERC721 standard](https://eips.ethereum.org/EIPS/eip-721).
...
- Reverts with `TokenDoesNotExist` error if the input id does not have an owner.
...
- Reverts with `TokenDoesNotExist` error if the input token id does not have an owner.
- Reverts with `NotOwnerNorApproved` error if the caller is not the owner or the approved address.
...
- Reverts with `TokenDoesNotExist` error if the input token id does not have an owner.
- Reverts with `TransferFromIncorrectOwner` error if the owner of input token id is not the same as the input owner address.
- Reverts with `TransferToZeroAddress` error if the input recipient address is the zero address.
- Reverts with `NotOwnerNorApproved` error if the caller is not the owner or the approved address.
- Reverts with `AccountBalanceOverflow` error if the balance of the input recipient address has overflowed.
...
- Reverts with `NotOwnerNorApproved` error if the caller is not the owner or the approved address.
- Reverts with `AccountBalanceOverflow` error if the balance of the input recipient address has overflowed.
- Reverts with `TransferToNonERC721ReceiverImplementer` error if the call to `onERC721Received` returns successfully with a value that is not `onERC721ReceivedSelector`.

...
- Reverts with the `BalanceQueryForZeroAddress` error if the input address is the zero address.
...
- Reverts with the `TokenDoesNotExist` error if the input token id does not have an owner.
...
- Reverts with the `TokenDoesNotExist` error if the input token id does not have an owner.
- Reverts with the `TransferFromIncorrectOwner` error if the owner of input token id is not the same as the input owner address.
